...
func Test_Handlers_Healthcheck_Drain_Undrain(t *testing.T) {
	...
	{ // Check health
		req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "http://localhost"+listenAddr+"/readyz", nil) //nolint:goconst,nolintlint
		require.NoError(t, err)
	...
	{ // Drain
		req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "http://localhost"+listenAddr+"/drain", nil)
		require.NoError(t, err)
		w := httptest.NewRecorder()
		start := time.Now()
		s.handleDrain(w, req)
	...
	{ // Check health
		req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "http://localhost"+listenAddr+"/readyz", nil)
		require.NoError(t, err)
		w := httptest.NewRecorder()
		s.handleReadinessCheck(w, req)
		resp := w.Result()
	...
	{ // Undrain
		req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "http://localhost"+listenAddr+"/undrain", nil)
		require.NoError(t, err)
		w := httptest.NewRecorder()
