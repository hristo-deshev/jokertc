...
build-cli: ## Build the CLI
	@mkdir -p ./build
	go build -trimpath -ldflags "-X jokertc/common.Version=${VERSION}" -v -o ./build/cli cmd/cli/main.go

.PHONY: build-server
build-server: ## Build the HTTP server
	@mkdir -p ./build
	go build -trimpath -ldflags "-X jokertc/common.Version=${VERSION}" -v -o ./build/server cmd/server/main.go
...
.PHONY: docker-server
docker-server: ## Build the HTTP server Docker image
	DOCKER_BUILDKIT=1 docker build \
		--platform linux/amd64 \
		--build-arg VERSION=${VERSION} \
